...
async function fetchResources() {
    try {
        const response = await fetch('http://localhost:8000/auth/resource_details');  // Update with the correct backend URL
        ...
        if (resources.msg) {
            ...
            resources.forEach(resource => {
                ...
                cellTitle.textContent = resource.resource_id;
                cellUserId.textContent = resource.link;
